...
SELECT
  ...
  i.`batch_no`,
  i.`quantity`,
  MIN(ird.`expiry_date`) AS `expiry_date`,
  DATEDIFF(MIN(ird.`expiry_date`), CURDATE()) AS `days_until_expiry`,
...
JOIN `inbound_receipt` ir
  ON ir.`warehouse_id` = i.`warehouse_id`
 AND ir.`status` = 'Completed'
JOIN `inbound_receipt_detail` ird
  ON ird.`receipt_id` = ir.`receipt_id`
 AND ird.`product_id` = i.`product_id`
 AND ird.`batch_no` = i.`batch_no`
WHERE i.`quantity` > 0
